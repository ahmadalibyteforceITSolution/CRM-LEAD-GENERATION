...
<template>
  ...
  >
    <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] flex flex-col overflow-hidden">
      ...
      <form @submit.prevent="handleSubmitCall" class="flex-1 overflow-y-auto p-6 space-y-5 text-xs">
        <!-- Live Call Stopwatch Banner -->
        <div class="p-4 rounded-xl bg-slate-900 text-white border border-slate-800 flex items-center justify-between shadow-inner">
          <div class="flex items-center gap-3">
            ...
            <div>
              ...
              <div class="text-2xl font-mono font-extrabold text-white tracking-widest">{{ formattedTimer }}</div>
            </div>
          </div>
          ...
          <div class="flex items-center gap-2">
            <a
              ...
              class="px-3 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-700 font-bold text-white flex items-center gap-1.5 transition-all shadow-md shadow-indigo-600/20"
            >
            ...
            <button
              ...
              @click="toggleTimer"
              :class="[
                'px-3 py-2 rounded-xl font-bold flex items-center gap-1.5 transition-all',
                isTimerRunning ? 'bg-amber-500 hover:bg-amber-600 text-slate-950' : 'bg-emerald-600 hover:bg-emerald-700 text-white'
              ]"
            >
              <component :is="isTimerRunning ? Pause : Play" class="w-4 h-4" />
            ...
            <button
              type="button"
              @click="resetTimer"
              class="p-2 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-300"
              title="Reset Timer"
            >
