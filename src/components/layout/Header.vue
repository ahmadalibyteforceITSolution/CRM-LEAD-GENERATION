  ...
  <header class="h-16 border-b border-slate-200 dark:border-slate-800 bg-white/95 dark:bg-slate-900/95 backdrop-blur px-6 flex items-center justify-between gap-4 sticky top-0 z-20">
    ...
    <div class="flex items-center gap-3">
      ...
      <button
        ...
        title="Hard Refresh directly from MongoDB Atlas Database"
        class="inline-flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-semibold bg-slate-100 hover:bg-slate-200 text-slate-700 dark:bg-slate-800 dark:hover:bg-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700"
      >
        <RotateCcw class="w-3.5 h-3.5 text-indigo-500" :class="{ 'animate-spin': store.isLoading }" />
        <span class="hidden md:inline">{{ store.isLoading ? 'Syncing...' : 'Sync DB' }}</span>
      </button>
